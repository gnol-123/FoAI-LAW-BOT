...
async function showChatView(user) {
  loginView.style.display  = "none";
  verifyView.style.display = "none";
...
function setAuthMode(mode) {
  ...
  const activeClass   = "bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm";
  const inactiveClass = "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200";
  tabSignin.className   = `flex-1 py-1.5 text-sm font-medium rounded-lg transition ${reg  ? inactiveClass : activeClass}`;
  tabRegister.className = `flex-1 py-1.5 text-sm font-medium rounded-lg transition ${reg  ? activeClass   : inactiveClass}`;
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  if (!user.emailVerified)  { showVerifyView(user); return; }
  await showChatView(user);
});
...
verifyCheckBtn.addEventListener("click", async () => {
  try {
    ...
    if (auth.currentUser.emailVerified) {
      await showChatView(auth.currentUser);
    } else {
